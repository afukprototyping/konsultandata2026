...
if is_dark:
    ...
    APP_BG = "#F8FAFC"; CARD_BG = "rgba(255,255,255,0.92)"; CARD_BD = "rgba(226,232,240,0.9)"
...
.stApp {{
    ...
    background-image: radial-gradient(circle at 10% 10%, rgba(232,80,10,0.04) 0%, transparent 40%),
...
def load_data():
    ...
    df2[col_id]  = df2[col_id].astype(str).str.strip().str.replace(r"\.0$","",regex=True).str.zfill(3)
    df2[col_nom] = pd.to_numeric(df2[col_nom], errors="coerce").fillna(0)

    col_id1 = _find_col(df1, "ID Klien", "ID Klien")
    if not col_id1: raise ValueError("Critical column 'ID Klien' missing in SPS 1.")
    df1['Generated_ID'] = df1[col_id1].astype(str).str.strip().str.replace(r"\.0$","",regex=True).str.zfill(3)
...
with col_b:
    ...
    if C_LAYAN and C_LAYAN in df_in.columns:
        ...
        fig.update_layout(
            ...
            annotations=[dict(text=f"<b>{n_in}</b><br>total", x=0.5, y=0.5,
                              font_size=16, font_color=T1, showarrow=False)]
        )
        ...
        for i, (_, r) in enumerate(svc.iterrows()):
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    <span style="color:{T1};font-size:0.82rem;font-weight:500;">{r['Service']}</span>
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    <span style="color:{T1};font-weight:700;font-size:0.82rem;">{int(r['Qty'])}</span>
                    <span style="color:{T2};font-size:0.75rem;min-width:32px;text-align:right;">{p}%</span>
                </div>
            </div>""", unsafe_allow_html=True)
    else:
        st.warning("N/A")
...
with col_c:
    ...
    if C_TOPIK and C_TOPIK in df_done.columns:
        ...
        t_rows = "".join(f"""<tr>
            ...
            <td style="text-align:right;font-weight:700;">{int(r['Qty'])}</td></tr>"""
            for _, r in topic_df.iterrows())

    st.markdown(f"""<div class="sc" style="max-height:{MH}px;overflow-y:auto;">
        <div class="sh">Topic Distribution</div>
        <table class="ct"><thead><tr><th>Topic</th><th style="text-align:right;">Qty</th></tr></thead>
